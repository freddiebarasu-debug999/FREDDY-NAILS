  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#d6b36a]/20 bg-[#11100f]/95 backdrop-blur-xl">
      ...
      {open && (
        <div
          id="mobile-menu"
          className="flex flex-col gap-3 border-t border-[#d6b36a]/20 bg-[#151311] px-5 pb-5 pt-3"
        >
          {LINKS.map((link) => (
            <a
              key={link.href}
          ...
          <a
            href="#booking"
            onClick={() => setOpen(false)}
            className="inline-block w-fit rounded-sm bg-[#d6b36a] px-5 py-[10px] text-xs font-bold uppercase tracking-wide text-[#11100f] transition-colors hover:bg-[#ad8a4e]"
          >
            Book now
          </a>
        </div>
