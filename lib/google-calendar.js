...
async function getGoogleAccessToken() {
  ...
  if (!tokenResponse.ok || !tokenData.access_token) {
    console.error("Google access token refresh failed:", tokenData);
...
function buildEventId(appointmentId) {
  return `freddynails${appointmentId.replace(/-/g, "")}`;
}
...
function buildCalendarEvent(appointment) {
  ...
  if (!appointment.booking_date || !startTime || !endTime) {
    throw new Error(
      "Appointment is missing booking date or start/end time."
    );
  }

  const clientCount = Number(appointment.client_count || 1);
  ...
  return {
    ...
    description: [
      ...
      "Payment status: Paid",
      `Booking status: ${appointment.booking_status || "Confirmed"}`,
...
export async function createGoogleCalendarEvent(appointment) {
  ...
  // If we already have an event ID, the appointment has already
  // been connected to Google Calendar.
  ...
  if (!response.ok) {
    // If the deterministic event ID already exists in Google,
    // treat it as already created instead of creating a duplicate.
    if (response.status === 409) {
      ...
      const { error: saveExistingError } = await supabaseAdmin
        .from("appointments")
        .update({
...
export async function deleteGoogleCalendarEvent(
  ...
  if (error) {
    throw new Error(
      `Calendar event was deleted, but the appointment could not be updated: ${error.message}`
    );
  }
