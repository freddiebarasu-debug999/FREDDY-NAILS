...
export default function LoginPage() {
  ...
  async function handleForgotPassword() {
    ...
    try {
      ...
      setMessage(
        "We've sent a password reset link to your email. Open the email and use the link to create a new password."
      );
    } catch (err) {
      console.error(
        ...
        >
          ...
          <div>
            <div className="mb-2 flex items-center justify-between gap-3">
              <label className="block text-xs font-bold uppercase tracking-[0.15em] text-[#c9c0b6]">
                Password
              </label>
              ...
              <button
                ...
                className="text-[0.7rem] font-semibold text-[#d6b36a] transition-colors hover:text-[#f0d18a] disabled:opacity-50"
              >
                {resetLoading
                  ? "Sending..."
                  : "Forgot password?"}
          ...
          {error && (
            <div className="border border-red-400/30 bg-red-400/10 px-4 py-3 text-sm leading-relaxed text-red-300">
              {error}
            </div>
          )}
