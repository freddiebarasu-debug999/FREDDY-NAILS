...
const TIME_ZONE = "Africa/Johannesburg"; const LOCATION = "8 Rhodes, Quigney, East London, Eastern Cape, South Africa";
function escapeICSText(value = "") { return String(value) .replace(/\/g, "\\") .replace(/\r?\n/g, "\n") .replace(/,/g, "\,") .replace(/;/g, "\;"); }
function formatICSDate(date, time) { const cleanTime = String(time || "00:00:00").slice(0, 8);
const [hours = "00", minutes = "00", seconds = "00"] = cleanTime.split(":");
return ${date.replace(/-/g, "")}T${hours.padStart( 2, "0" )}{minutes.padStart(2, "0")}{seconds.padStart(2, "0")}; }
function formatICSDateUTC(date = new Date()) { return ( date.toISOString().replace(/[-:]/g, "").replace(/.\d{3}/, "") + "Z" ); }
export async function GET(request) { try { const url = new URL(request.url); const appointmentId = url.searchParams.get("appointment");
if (!appointmentId) {
...
} catch (error) { console.error("Calendar file error:", error);
